...
The reasoning behind Claude Stats' behaviour — why the readout looks the way it does,
where the numbers come from, and why the app never touches a refresh token.
...
every Claude gauge and no OpenAI ones. When gauges from both providers are visible at
once, each group carries a small `CC` / `GPT` tag, since both providers have a
session and a weekly quota and the letters alone would stop identifying anything.
...
The app **only reads** access tokens, and re-reads them on every poll. Claude Code
and the Codex CLI own their refresh cycles and write fresh tokens back to the same
places, so this app stays current without ever touching a refresh token or writing
to your credentials.

That ownership is deliberate, not laziness. Both token endpoints return a new
`refresh_token` alongside each access token — refresh tokens **rotate** — so a second
client performing its own refresh would invalidate the one the CLI holds and break
your actual login. There is no safe way to have two independent owners of one
credential.

There is also no keychain permission dialog. The app reads the Claude token by
running `/usr/bin/security find-generic-password` — the same binary Claude Code
writes the item through — so `security` is already on the item's access list and
macOS never asks. Reading via the Security framework instead would prompt on every
